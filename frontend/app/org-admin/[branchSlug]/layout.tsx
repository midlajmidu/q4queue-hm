...
/**
 * Layout for the org-admin read-only branch view.
 * Mirrors the standard [orgSlug]/dashboard/layout.tsx but
 * derives the dashboard base path from `branchSlug` param.
 */
export default function OrgAdminBranchLayout({ children }: { children: ReactNode }) {
    const [isMobileMenuOpen, setIsMobileMenuOpen] = useState(false);
    const [isSidebarCollapsed, setIsSidebarCollapsed] = useState(false);
    const { user, logout } = useAuth();
    const pathname = usePathname();
    const params = useParams();
    // Use branchSlug from URL, falling back to user's org_slug from token
    const branchSlug = (params?.branchSlug as string) || user?.org_slug;
    const dashBase = branchSlug ? `/org-admin/${branchSlug}/dashboard` : "/organization-admin";
