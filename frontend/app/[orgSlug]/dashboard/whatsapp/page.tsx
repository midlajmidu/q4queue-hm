                ...
                {tabs.map((tab) => (
                    <button
                        ...
                        className={`pb-3 text-sm font-medium transition-colors border-b-2 ${
                            activeTab === tab.id 
                            ? "border-indigo-600 text-indigo-600 dark:border-indigo-500 dark:text-indigo-600 dark:text-indigo-400" 
            ...
            <div className="pt-4">
                {activeTab === "overview" && (
                    <div className="space-y-6">
                        <div className="bg-indigo-50 dark:bg-indigo-900/40 border border-indigo-200 dark:border-indigo-500/30 rounded-xl p-5 flex items-start gap-4">
                            ...
                            <div>
                                <h3 className="text-indigo-900 dark:text-indigo-200 font-bold text-sm">Hybrid Notification Model Active</h3>
                                <p className="text-indigo-800 dark:text-indigo-300/80 text-xs mt-1 leading-relaxed">
                ...
                {activeTab === "events" && (
                    <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl overflow-hidden">
                        <table className="w-full text-left text-sm">
                            <thead className="bg-slate-50 dark:bg-slate-800/50 text-slate-500 dark:text-slate-500 dark:text-slate-400 text-xs uppercase">
                                <tr>
                            ...
                            <tbody className="divide-y divide-slate-100 dark:divide-slate-800/50 text-slate-700 dark:text-slate-300">
                                {eventStats.length === 0 ? (
                                    ...
                                    <tr key={i} className="hover:bg-slate-50 dark:hover:bg-slate-800/20">
                                        ...
                                        <td className="px-6 py-3 text-emerald-600 dark:text-emerald-400">{s.delivered}</td>
                                        <td className="px-6 py-3 text-indigo-600 dark:text-indigo-400">{s.read}</td>
                                        <td className="px-6 py-3 text-red-600 dark:text-red-400">{s.failed}</td>
                                    ...
                                    {logs?.items.length === 0 ? (
                                        ...
                                        <tr key={m.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/20 transition-colors">
                                            <td className="px-6 py-3">
                                                ...
                                                <div className="font-mono text-xs text-slate-500 dark:text-slate-400 mt-0.5">{m.customer_phone}</div>
                                            </td>
                                            <td className="px-6 py-3">
                                                <span className="bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 px-2.5 py-1 rounded-md text-[11px] font-medium">
                                            ...
                                            <td className="px-6 py-3">
                                                ...
                                                {m.error_message && (
                                                    <div className="text-[10px] text-red-600 dark:text-red-400 mt-1 max-w-[200px] truncate" title={m.error_message}>
                                                        {m.error_message}
                                                    </div>
                                                )}
                                            </td>
                ...
                {activeTab === "settings" && (
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        {/* Global Enable */}
                        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl p-6 col-span-1 md:col-span-2">
                            <h2 className="text-lg font-bold text-slate-900 dark:text-white mb-4">Master Switch</h2>
                            <label className="flex items-center gap-3 cursor-pointer">
                                <div className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${config?.is_enabled ? "bg-[#25d366]" : "bg-slate-300 dark:bg-slate-700"}`}>
                        ...
                        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl p-6">
                            <h2 className="text-lg font-bold text-slate-900 dark:text-white mb-2">Notification Pipeline</h2>
                            <p className="text-sm text-slate-500 dark:text-slate-400 mb-6">Manage how notifications flow to your customers through the hybrid pipeline.</p>
                            
                            {/* Section 1: Paid Template */}
                            <div className="bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700/50 rounded-lg p-5 mb-6">
                                <div className="flex items-center gap-2 mb-1">
                                    <h3 className="font-bold text-emerald-600 dark:text-emerald-400 text-sm uppercase tracking-wide">Step 1: Primary Gateway</h3>
                                    <span className="bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 text-[10px] px-2 py-0.5 rounded border border-emerald-500/20">Paid Template</span>
                                </div>
                                <p className="text-xs text-slate-500 dark:text-slate-400 mb-4">This initial welcome message uses an official Meta Utility template to establish the 24-hour service window.</p>
                                
                                <label className="flex items-center justify-between cursor-pointer">
                                    <div>
                                        <div className="text-sm font-medium text-slate-700 dark:text-slate-200">Queue Joined</div>
                                        <div className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">Sent immediately when a customer is added to a queue. Includes the quick-reply opt-in button.</div>
                                    </div>
                                    <div className={`relative inline-flex h-5 w-9 items-center rounded-full transition-colors ${config?.notify_queue_joined ? "bg-[#25d366]" : "bg-slate-300 dark:bg-slate-700"}`}>
                            ...
                            <div className="bg-indigo-50/50 dark:bg-indigo-900/20 border border-indigo-100 dark:border-indigo-500/20 rounded-lg p-5">
                                <div className="flex items-center gap-2 mb-1">
                                    <h3 className="font-bold text-indigo-400 text-sm uppercase tracking-wide">Step 2: Free Hybrid Follow-ups</h3>
                                    <span className="bg-indigo-100 dark:bg-indigo-500/10 text-indigo-700 dark:text-indigo-400 text-[10px] px-2 py-0.5 rounded border border-indigo-200 dark:border-indigo-500/20">100% Free</span>
                                </div>
                                <p className="text-xs text-slate-500 dark:text-slate-400 mb-5">These updates are delivered as raw text completely free of charge. <strong>Note:</strong> They are only delivered if the user actively opted in via the Quick Reply button on the welcome message.</p>

                                <div className="space-y-4">
                                    <label className="flex items-center justify-between cursor-pointer">
                                        <div>
                                            <div className="text-sm font-medium text-slate-700 dark:text-slate-200">Position 5 Reminder</div>
                                            <div className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">Alerts customer when there are exactly 5 people ahead.</div>
                                        </div>
                                        <div className={`relative inline-flex h-5 w-9 items-center rounded-full transition-colors ${config?.notify_position_5 ? "bg-indigo-500" : "bg-slate-300 dark:bg-slate-700"}`}>
                                    ...
                                    <label className="flex items-center justify-between cursor-pointer">
                                        <div>
                                            <div className="text-sm font-medium text-slate-700 dark:text-slate-200">Position 3 Reminder</div>
                                            <div className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">Alerts customer when there are exactly 3 people ahead.</div>
                                        </div>
                                        <div className={`relative inline-flex h-5 w-9 items-center rounded-full transition-colors ${config?.notify_position_3 ? "bg-indigo-500" : "bg-slate-300 dark:bg-slate-700"}`}>
                                    ...
                                    <label className="flex items-center justify-between cursor-pointer">
                                        <div>
                                            <div className="text-sm font-medium text-slate-700 dark:text-slate-200">Customer Called</div>
                                            <div className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">Sent when it is their turn to proceed to the counter.</div>
                                        </div>
                                        <div className={`relative inline-flex h-5 w-9 items-center rounded-full transition-colors ${config?.notify_called ? "bg-indigo-500" : "bg-slate-300 dark:bg-slate-700"}`}>
                                    ...
                                    <label className="flex items-center justify-between cursor-pointer">
                                        <div>
                                            <div className="text-sm font-medium text-slate-700 dark:text-slate-200">Service Completed</div>
                                            <div className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">Thank you message and review request when service finishes.</div>
                                        </div>
                                        <div className={`relative inline-flex h-5 w-9 items-center rounded-full transition-colors ${config?.notify_completed ? "bg-indigo-500" : "bg-slate-300 dark:bg-slate-700"}`}>
                        ...
                        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl p-6 flex flex-col justify-between">
                            <div>
                                <h2 className="text-lg font-bold text-slate-900 dark:text-white mb-4">Send Test Notification</h2>
                                ...
                                <div className="space-y-4">
                                    <input
                                        ...
                                        className="w-full bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg px-4 py-3 text-sm text-slate-900 dark:text-white focus:outline-none focus:border-indigo-500 font-mono"
                                    />
                                    <button
                                        onClick={sendTest}
                                ...
                                {testMsg && <div className={`text-sm mt-4 font-medium ${testMsg.includes("✓") ? "text-emerald-600 dark:text-emerald-400" : "text-red-600 dark:text-red-400"}`}>{testMsg}</div>}
                            </div>
                        </div>
                    </div>
                )}
