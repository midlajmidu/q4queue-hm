...
/**
 * Layout for the super-admin read-only branch view.
 * Mirrors the standard [orgSlug]/dashboard/layout.tsx but
 * derives the dashboard base path from url params.
 */
export default function SuperAdminBranchLayout({ children }: { children: ReactNode }) {
    ...
    // Fallback logic for URL params
    const parentOrgSlug = (params?.parentOrgSlug as string) || "system";
    const branchSlug = (params?.branchSlug as string) || user?.org_slug;
    const dashBase = branchSlug ? `/super-admin/${parentOrgSlug}/${branchSlug}/dashboard` : "/super-admin";
