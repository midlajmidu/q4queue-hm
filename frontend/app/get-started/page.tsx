...
export default function GetStartedPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!formData.fullName || !formData.workEmail || !formData.organization) return;
    setSubmitting(true);
    setSubmitError(null);
    try {
      await api.submitPublicCustomPlanRequest({
        ...
        branch_count: "1 – 3 Branches",
        queue_count: "1 – 3 Queues",
        staff_count: "1 – 5 Staff",
        visitor_volume: formData.dailyVisitors,
        selected_services: ["Instant Pilot Sandbox"],
        special_notes: formData.notes,
      });
      setSubmitted(true);
    } catch (err: any) {
      setSubmitError(err?.detail || "Unable to submit pilot request. Please check your network.");
    } finally {
      setSubmitting(false);
    }
  };
      ...
      <main className="pt-24 sm:pt-28 pb-20 sm:pb-28">
        <div className="max-w-7xl mx-auto px-6 lg:px-8">
          ...
          <div className="text-center max-w-3xl mx-auto mt-6 sm:mt-10">
            <div className="inline-flex items-center px-3.5 py-1 rounded-full bg-blue-50 border border-blue-200 text-xs font-semibold text-blue-700 mb-4">
              <span>Instant Pilot &amp; Architecture Review</span>
            </div>
            <h1 className="text-3xl sm:text-4xl lg:text-5xl font-extrabold text-[#0B1220] tracking-tight leading-tight">
          ...
          <div className="mt-12 max-w-5xl mx-auto grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-12 items-start">
            ...
            <div className="lg:col-span-7 bg-white rounded-2xl border border-slate-200 p-8 sm:p-10 shadow-xs">
              {submitted ? (
                ...
                <form onSubmit={handleSubmit} className="space-y-5">
                  <h3 className="text-xl font-bold text-[#0B1220] tracking-tight mb-2">
                  ...
                  <button
                    type="submit"
                    className="w-full h-12 rounded-xl bg-blue-600 hover:bg-blue-700 text-white font-bold text-sm shadow-xs transition-all flex items-center justify-center gap-2 cursor-pointer"
                  >
            ...
            {/* Right Column: Clean White Minimal Info Container (5 cols) */}
            <div className="lg:col-span-5 space-y-6">
              <div className="rounded-2xl bg-white text-slate-900 p-8 border border-slate-200 shadow-xs">
                <div className="text-xs font-bold text-blue-600 uppercase tracking-wider mb-2">
                  What you get
                </div>
                <h4 className="text-xl font-bold tracking-tight text-[#0B1220]">
              ...
              <div className="rounded-2xl bg-white border border-slate-200 p-6 shadow-xs">
                ...
                <div className="mt-3.5 space-y-2 text-xs font-medium">
                  <div className="flex items-center gap-2 text-blue-600 font-semibold">
                    ...
                    <span>contact@q4queue.com</span>
                  </div>
                  <div className="flex items-center gap-2 text-slate-700 font-semibold">
                    <Phone className="w-4 h-4 text-slate-400" />
                    <span>+91 95396 79027</span>
                  </div>
                </div>
              </div>
            </div>
