...
def _require_customer_data_access(user: User) -> None:
    if user.role not in {"admin", "branch_admin", "organization_admin", "super_admin"}:
        raise HTTPException(
            status_code=http_status.HTTP_403_FORBIDDEN,
            detail="Administrator access is required for customer history and exports.",
        )
...
async def get_overview(
    ...
    recent_limit: int = Query(5, ge=1, le=100, description="Number of recent activities to show"),
    recent_offset: int = Query(0, ge=0, description="Offset for recent activities"),
...
async def get_history(
    ...
    import logging
    logger = logging.getLogger(__name__)
    _require_customer_data_access(current_user)
    if not current_user.org_id:
    ...
    try:
        ...
        raise HTTPException(
            status_code=http_status.HTTP_503_SERVICE_UNAVAILABLE,
            detail="Customer history is temporarily unavailable. Please try again.",
        ) from exc
...
async def export_analytics_csv(
    ...
    _require_customer_data_access(current_user)
    if not current_user.org_id:
        raise HTTPException(status_code=http_status.HTTP_403_FORBIDDEN, detail="Branch context is required.")
    try:
